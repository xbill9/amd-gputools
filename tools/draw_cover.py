...
S = 2; W, H = 1376, 578
SURFACE=(26,26,25); INK=(255,255,255); MUTED=(158,158,153); RULE=(58,58,55)
ORANGE=(217,89,38); BLUE=(57,135,229); GOLD=(198,156,62); PCB=(28,74,48)
BLACK=(20,20,20); WHITE=(250,250,248); BEAK=(232,154,40)
SANS_B="/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf"
SANS  ="/usr/share/fonts/truetype/liberation/LiberationSans-Regular.ttf"
MONO  ="/usr/share/fonts/truetype/liberation/LiberationMono-Regular.ttf"
f=lambda p,pt: ImageFont.truetype(p, pt*S)
im=Image.new("RGB",(W*S,H*S),SURFACE); d=ImageDraw.Draw(im)
def B(xy,**k): d.rectangle([c*S for c in xy],**k)
def P(p,**k):  d.polygon([(x*S,y*S) for x,y in p],**k)
def E(xy,**k): d.ellipse([c*S for c in xy],**k)
def T(xy,s,fnt,fill): d.text((xy[0]*S,xy[1]*S),s,font=fnt,fill=fill)
def w_(s,fnt): return d.textlength(s,font=fnt)/S

# ---------------- left column ----------------
T((72,92),"AMD INSTINCT MI300X  ·  gfx942  ·  CDNA 3",f(MONO,14),MUTED)
hl=f(SANS_B,41)
for i,l in enumerate(["What the matrix cores","execute, and what they don't"]):
    assert w_(l,hl)<=560, (l,w_(l,hl)); T((72,136+i*52),l,hl,INK)
T((72,256),"304 CUs · 191.69 GiB HBM3 · one $1.99/hr card",f(SANS,19),MUTED)
def chip(x,y,label,num,unit,colour):
    B((x,y,x+4,y+54),fill=colour); T((x+16,y),label,f(MONO,13),MUTED)
    nf=f(SANS_B,31); T((x+16,y+19),num,nf,INK)
...
for i in range(28):
    x=X0+26+i*15
    if x+5<X0+CW-16: B((x,Y0+18,x+5,Y0+CH-16),fill=(62,62,60))
for x0 in (X0+64,X0+166):
    P([(x0,Y0-4),(x0+74,Y0-4),(x0+74+SK-10,Y0-SK+8),(x0+SK-10,Y0-SK+8)],fill=(76,76,73))
B((X0-4,Y0+CH,X0+CW+4,Y0+CH+12),fill=PCB)
for i in range(24):
    x=X0+42+i*16
    if x+9<X0+CW-18: B((x,Y0+CH+12,x+9,Y0+CH+32),fill=GOLD)
B((X0-22,Y0-SK-6,X0-4,Y0+CH+30),fill=(70,70,67))
for i in range(4): B((X0-19,Y0-10+i*30,X0-7,Y0+8+i*30),fill=(30,30,29))

# ---------------- Tux: beside the card, left flipper up on it ----------------
tx,ty,s = 1206,352,1.22
sx=lambda v: tx+v*s; sy=lambda v: ty+v*s
...
for ex in (-22,4): E((sx(ex),sy(-97),sx(ex+18),sy(-75)),fill=WHITE)
for ex in (-15,9): E((sx(ex),sy(-91),sx(ex+8),sy(-81)),fill=(16,16,16))
P([(sx(-14),sy(-74)),(sx(14),sy(-74)),(sx(0),sy(-56))],fill=BEAK)        # beak
for fx in (-34,8):                                                        # feet
    P([(sx(fx),sy(78)),(sx(fx+40),sy(70)),(sx(fx+34),sy(96)),(sx(fx-6),sy(96))],fill=BEAK)

im.resize((W,H),Image.LANCZOS).save("/tmp/cover-art.png"); print("ok")
